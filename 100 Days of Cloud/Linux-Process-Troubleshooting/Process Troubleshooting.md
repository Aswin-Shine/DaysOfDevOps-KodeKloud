...
They might not hosted any code yet on these servers so you need not to worry about if Apache isn't serving any pages or not, just make sure service is up and running. Also, make sure Apache is running on port 5000 on all app servers.
...
sudo kill <726> -> This was for my case
sudo systemctl start httpd
sudo systemctl status httpd
