...
1. Create an S3 bucket named devops-stream-bucket-30971 using Terraform.
...
5. Create a Firehose delivery stream named devops-firehose-stream to deliver data to the S3 bucket.
...
variable "KKE_S3_BUCKET_NAME" {
  ...
  type        = string
  default     = "devops-stream-bucket-30971"
}
...
resource "aws_kinesis_firehose_delivery_stream" "devops_stream" {
...
output "kke_firehose_stream_name" {
  value       = aws_kinesis_firehose_delivery_stream.devops_stream.name
  description = "The name of the created Firehose stream"
}
...
5. To verify the task use this command

```
aws firehose put-record --delivery-stream-name devops-firehose-stream --record '{"Data":"DevOps-Test-Data"}'
```
