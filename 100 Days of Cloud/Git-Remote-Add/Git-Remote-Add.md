The xFusionCorp development team added updates to the project that is maintained under `/opt/demo.git` repo and cloned under `/usr/src/kodekloudrepos/cluster`.
...
a. In `/usr/src/kodekloudrepos/cluster` repo add a new remote `dev_cluster` and point it to `/opt/xfusioncorp_demo.git` repository.
